...
use amethyst::ui::{UiTransform, UiText, UiImage, TextEditing, FontAsset, Interactable, Selectable, Selected, FontHandle};
use amethyst::assets::{Loader, AssetStorage};
use amethyst::winit::VirtualKeyCode;
use crate::developer_console_utils;
...
impl <'s> System<'s> for DeveloperConsoleSystem {
    ...
    fn run(&mut self, (loader, font_assets, input, mut console_resource, ui_transform, interactables, selectables, ui_images, mut ui_texts, text_editings, selected, entities): Self::SystemData) {


        // Handle enter on input
        if console_resource.displayed {
            let mut input_text = None;
            for (entity, transform, _editing) in (&*entities, &ui_transform, &text_editings).join() {
                if transform.id.starts_with( "developer-console") {
            ...
            if let Some(e) = input_text {
                for (entity, _selected, text) in (&*entities, &selected, &mut ui_texts).join() {
                    if entity.eq(&e) {
